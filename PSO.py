...
def get_particles_accuracy(particles_position, ann: ANN):
    particles_accuracy = np.array([])
    
    for i in range(len(particles_position)):
        ann.fill_weights(particle=particles_position[i])
        ann.forward_pass()
        particles_accuracy = np.append(particles_accuracy, ann.accuracy)
...
def get_particles_cost(particles_position, ann: ANN):
    ...
    for i in range(len(particles_position)):
        ann.fill_weights(particle=particles_position[i])
        ann.forward_pass()
        particles_cost = np.append(particles_cost, ann.cost)
...
def pso_accuracy(num_particles: int, ann: ANN, max_iter: int, **kwargs):
    np.random.seed(kwargs.get("seed"))
    particles_inertia = np.random.rand(num_particles, ) if kwargs.get("particles_inertia") is None\
                        else np.full(num_particles, kwargs.get("particles_inertia"))
    c1 = np.random.rand(1,)[0] if  kwargs.get("c1") is None else kwargs.get("c1")
    ...
    particles_position_pbest = np.array(particles_position, copy=True)
    # particles_position_pbest_cost =  get_particles_cost(particles_position=particles_position_pbest, ann=ann)
    particles_position_pbest_accuracy = get_particles_accuracy(particles_position=particles_position_pbest, ann=ann)
    
    particle_position_gbest = particles_position[np.argmax(particles_accuracy)]
    particle_position_gbest_cost = get_particle_cost(particle_position_gbest, ann=ann)
    particle_position_gbest_accuracy = get_particle_accuracy(particle_position_gbest, ann=ann)
